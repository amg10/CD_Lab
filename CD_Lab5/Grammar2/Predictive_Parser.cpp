...
vector<string> terminals={"prog","end","+","-","*","/","=",">","<","(",")","{","}",":=",";","and","else","ic","id","if","int","float","do","fc","not","or","print","scan","str","then","while","$"};
...
{
	fstream f;
	f.open("Parsing_Table.txt",ios::out);
	cout<<"\nParsing Table:";
	cout<<"\n";
	cout<<"\t";
	f<<"\nParsing Table:\n\t";
...
void parse()
{
	...
	//cout<<pdstack.size()<<"\t"<<remaining_input[next_inp];
...
int main()
{
	...
	cout<<"\n\nParsing:\nCurrent Input\tCurrent stack top\tNext Action\n";
	parse();
	return 0;
}